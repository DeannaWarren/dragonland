...
var displayResults = function(event){
	...
	var colorIndex = {
		dd1: dragonColors.raw.indexOf(dataholder.dd1),
		dd2: dragonColors.raw.indexOf(dataholder.dd2),
		dd3: dragonColors.raw.indexOf(dataholder.dd3),
		p1: dragonColors.raw.indexOf(dataholder.p1),
		p2: dragonColors.raw.indexOf(dataholder.p2),
		p3: dragonColors.raw.indexOf(dataholder.p3) 
	};
	var colorTester = function(desiredColor,fixedColor, type){
		if(desiredColor == fixedColor){
			return type + " color is the same!";
		} else if (desiredColor < fixedColor){
			return "DD's color is higher on list than parent's";
		} else if (desiredColor > fixedColor){
			return "DD's color is lower on list than parent's";
		} else {
			return "something isn't quite right here?"
		};
	};

	var results = [
		"<h2> Your Input </h2> <p> Dream Dragon: ",
		dataholder.dd1,
		" / ",
		dataholder.dd2,
		" / ",
		...
		" / ",
		dataholder.p3,
		"</p> <h2> Hypothetical 2nd parent: </h2> <p> Primary Color Range: ",
		colorTester(colorIndex.dd1,colorIndex.p1,"primary"),
		" </p> <p> Secondary Color Range: ",
		colorTester(colorIndex.dd2,colorIndex.p2,"secondary"),
		" </p> <p> Tertiary Color Range: ",
		colorTester(colorIndex.dd3,colorIndex.p3,"tertiary"),
		" </p> "
		].join("")
...
var fillFormOptions = function(){
	// this function allows me to keep my HTML clean while also using dragonColors.raw, which I would need anyways for later functions
...
// overly complicated way to avoid having to manually check color list TWICE for discrepancy. (Manually added 3 colors to .raw)
var testColors = function(array,nestedArray){
	var unNest = [];
	var arrayLength = dragonColors.hex.length;
	for (var i = 0; i < arrayLength; i++) {
		unNest[i] = dragonColors.hex[i][0];
	};
	var errorFlag = false;
	for (var i=0; i < dragonColors.raw.length && errorFlag == false; i++){
		if (unNest[i] != dragonColors.raw[i]){
			errorFlag = true;
			console.log("Color is missing hex! :: " + dragonColors.raw[i])
		};
	};
	if (errorFlag == false){
		console.log("Congrats! Colors synced.")
	} else {
		console.log("Oops, there's an issue. Fix the above color.")
	};
};